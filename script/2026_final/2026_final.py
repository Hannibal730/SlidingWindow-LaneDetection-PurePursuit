...
try:
    ...
    # Backward compatibility for older NumPy versions
...
# 플롯 세팅
###############################################################q##########################
fig, ax = plt.subplots(figsize=(8, 7), dpi=100, constrained_layout=False)


ax.grid(True)
ax.set_xlim(-1.2, 1.2)  # Lateral axis range
ax.set_ylim(0, 2.0+1.341)  # Forward axis range
ax.set_aspect('equal', adjustable='box')
fig.suptitle("Lane Points and Polynomial Fitting in Meter Coordinates", y=0.98)
ax.set_xlabel("Lateral Position (m)")
ax.set_ylabel("Forward Position (m)")

rear_axle_center_size = 0.07 # 차량의 후륜축 중심 위치로 설정할 네모 
rear_axle_center = plt.Rectangle((-0.5*rear_axle_center_size, 0), rear_axle_center_size, rear_axle_center_size, linewidth=1, edgecolor='purple', facecolor='purple')
...
L_fit_line, = ax.plot([], [], '-', color="blue", label= "Left Lane Fitting")

R_scatter, = ax.plot([], [], 'ro', label="Right Lane Points")
R_fit_line, = ax.plot([], [], '-', color="red", label= "Right Lane Fitting")



p_scatter, = ax.plot([], [], 'o', color = 'green', label="Path Points")
p_fit_line, = ax.plot([], [], '-', color="green", label= "Path Fitting")

nv_p_scatter, = ax.plot([], [], 'o', color = 'black', label="nv Path Points")
nv_fit_line, = ax.plot([], [], '-', color="black", label= "nv Path Fitting")

Lookahead_point, = ax.plot([], [], 'o', color='orange', markeredgecolor='black', markersize=10, label="Lookahead Point")



plt.subplots_adjust(left=0.07, right=0.58)
steering_text = ax.text(
    1.38, 0.30, "steering angle (deg)\nN/A",
...
lookahead_circle = plt.Circle(
    (0, 0), 2.5, fill=False, edgecolor='gray', linestyle='--', linewidth=1.5, alpha=0.9
)
ax.add_patch(lookahead_circle)
ld_legend_handle = Line2D([0], [0], color='gray', linestyle='--', linewidth=1.5, label='Lookahead Distance')
legend_handles, legend_labels = ax.get_legend_handles_labels()
legend_handles.append(ld_legend_handle)
legend_labels.append("Lookahead Distance")
if "Lookahead Point" in legend_labels and "Lookahead Distance" in legend_labels:
    lp_idx = legend_labels.index("Lookahead Point")
    ld_idx = legend_labels.index("Lookahead Distance")
    legend_handles[lp_idx], legend_handles[ld_idx] = legend_handles[ld_idx], legend_handles[lp_idx]
    legend_labels[lp_idx], legend_labels[ld_idx] = legend_labels[ld_idx], legend_labels[lp_idx]
ax.legend(legend_handles, legend_labels, loc='upper left', bbox_to_anchor=(1.38, 1), borderaxespad=0.)




#########################################################################################
# 픽셀 좌표계에서 차량의 위치(541/2, 480)가 미터 좌표계 (0, 0)에 대응하게 설정
#########################################################################################
...
def pixeL_to_meter(x_pixel, y_pixel, origin_x, origin_y, S_x, S_y):
    ...
    return y_meter, -x_meter # 차량 기준 좌표(전방/좌우)로 맞추기 위해 축 회전/부호 변환한 결과

def vehicle_to_plot_coords(x_vehicle, y_vehicle):
    # Vehicle frame (forward=x, lateral=y) -> plot frame (x=-lateral, y=forward)
    return -y_vehicle, x_vehicle
...
def histogram_to_image(histogram, L_peak_x, R_peak_x, L_threshold, R_threshold, height=260):
    hist = np.asarray(histogram, dtype=np.float32)
    width = hist.shape[0]
    hist_img = np.zeros((height, width, 3), dtype=np.uint8)

    if hist.max() > 0:
        norm = (hist / hist.max()) * (height - 20)
        y_vals = (height - 1 - norm).astype(np.int32)
        pts = np.column_stack((np.arange(width, dtype=np.int32), y_vals))
        cv2.polylines(hist_img, [pts], isClosed=False, color=(180, 180, 180), thickness=1)

        cv2.line(hist_img, (L_threshold, 0), (L_threshold, height - 1), (255, 0, 0), 1)
        cv2.line(hist_img, (R_threshold, 0), (R_threshold, height - 1), (0, 0, 255), 1)

        if 0 <= L_peak_x < width:
            cv2.circle(hist_img, (L_peak_x, y_vals[L_peak_x]), 4, (255, 0, 0), -1)
        if 0 <= R_peak_x < width:
            cv2.circle(hist_img, (R_peak_x, y_vals[R_peak_x]), 4, (0, 0, 255), -1)

    cv2.putText(hist_img, "Histogram", (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 1, cv2.LINE_AA)
    return hist_img
...
#########################################################################################
def R_Polyft_Plotting(R_points_meter_list, R_scatter, R_fit_line):
    R_x, R_y = zip(*R_points_meter_list)
    R_plot_x, R_plot_y = vehicle_to_plot_coords(np.array(R_x), np.array(R_y))
    R_scatter.set_data(R_plot_x, R_plot_y)
    # 일반 다항식 피팅 (3차 다항식)
    ...
    R_fit_plot_x, R_fit_plot_y = vehicle_to_plot_coords(R_x_fit, R_y_fit)
    R_fit_line.set_data(R_fit_plot_x, R_fit_plot_y)
    return R_poly_func


def L_Polyft_Plotting(L_points_meter_list, L_scatter, L_fit_line):
    L_x, L_y = zip(*L_points_meter_list)
    L_plot_x, L_plot_y = vehicle_to_plot_coords(np.array(L_x), np.array(L_y))
    L_scatter.set_data(L_plot_x, L_plot_y)
    # 일반 다항식 피팅 (1차 다항식)
    ...
    L_fit_plot_x, L_fit_plot_y = vehicle_to_plot_coords(L_x_fit, L_y_fit)
    L_fit_line.set_data(L_fit_plot_x, L_fit_plot_y)
    return L_poly_func




def p_Polyft_Plotting(p_list, p_scatter, p_fit_line):
    if len(p_list) ==0:
        return None
    p_x, p_y = zip(*p_list)
    p_plot_x, p_plot_y = vehicle_to_plot_coords(np.array(p_x), np.array(p_y))
    p_scatter.set_data(p_plot_x, p_plot_y)
    # 일반 다항식 피팅 (3차 다항식)
    ...
    p_fit_plot_x, p_fit_plot_y = vehicle_to_plot_coords(p_x_fit, p_y_fit)
    p_fit_line.set_data(p_fit_plot_x, p_fit_plot_y)
    return p_poly_func


def nv_p_Polyft_Plotting(nv_p_list, nv_p_scatter, nv_fit_line, N):
    if len(nv_p_list) == 0:
        return None
    global nv_p_x
    nv_p_x, nv_p_y = zip(*nv_p_list)
    nv_plot_x, nv_plot_y = vehicle_to_plot_coords(np.array(nv_p_x), np.array(nv_p_y))
    nv_p_scatter.set_data(nv_plot_x, nv_plot_y)
    # 일반 다항식 피팅 (3차 다항식)
    ...
    nv_fit_plot_x, nv_fit_plot_y = vehicle_to_plot_coords(nv_p_x_fit, nv_p_y_fit)
    nv_fit_line.set_data(nv_fit_plot_x, nv_fit_plot_y)
...
# 접선벡터에 수직인 단위벡터를 구하고, 그 값에 평행이동 거리(0.425m)를 반영해 경로점 생성
#########################################################################################
...
# 윈도우 제작 후 프레임 반복 시작
#########################################################################################
cap = cv2.VideoCapture("video/dolbat_toyproject.mp4")
...
cv2.namedWindow("Polynomial Fit Plot", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Polynomial Fit Plot", 800, 700)
cv2.namedWindow("Histogram", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Histogram", 800, 260)
canvas = FigureCanvas(fig) # FigureCanvas 객체는 한 번만 생성
canvas_width, canvas_height = canvas.get_width_height()
plot_shape = (canvas_height, canvas_width, 4)
paused = False
...
while True:
    if not paused:
        ...
        prev_frame_time = now
    fps_text.set_text(f"FPS: {smoothed_fps:.1f}")
    steering_text.set_text("steering angle (deg)\nN/A")
    lookahead_circle.set_radius(Ld)
    ...
    histogram_img = histogram_to_image(histogram, L_x_hist_argmax, R_x_hist_argmax, L_x_threshold, R_x_threshold)
    ...
    for window in range(R_num_windows):
        ...
        if len(R_win_indices) > min_points:
            ...
            R_poly_func = R_Polyft_Plotting(R_points_meter_list, R_scatter, R_fit_line) # 오른쪽 차선 피팅용 함수 불러오기       
    ...
    for window in range(L_num_windows):              
        ...
        if len(L_win_indices) > min_points:
            ...
            L_poly_func = L_Polyft_Plotting(L_points_meter_list, L_scatter, L_fit_line)
    ...
    if len(p_list) > 0: # 초록차선 피팅
        p_poly_func = p_Polyft_Plotting(p_list, p_scatter, p_fit_line)
        
    if len(nv_p_list) > 0: # 오른쪽 차선 데이터가 없다면(다시말해, 왼쪽 차선 데이터만 있다면) 1차 피팅, 있다면 3차 피팅
        if len(R_points_meter_list) == 0: N =1
        else: N = 3
        nv_p_poly_func = nv_p_Polyft_Plotting(nv_p_list, nv_p_scatter, nv_fit_line, N)
    ...
    if lookahead_point_list is not None:
        for lookahead_point in lookahead_point_list:
            lookahead_plot_x, lookahead_plot_y = vehicle_to_plot_coords(lookahead_point[0], lookahead_point[1])
            x_plot = float(np.asarray(lookahead_plot_x).reshape(-1)[0])
            y_plot = float(np.asarray(lookahead_plot_y).reshape(-1)[0])
            Lookahead_point.set_data([x_plot], [y_plot])
            ...
            steering_text.set_text(f"steering angle (deg)\n{delta_degrees:.2f}")
            x_val = float(np.asarray(lookahead_point[0]).reshape(-1)[0])
            y_val = float(np.asarray(lookahead_point[1]).reshape(-1)[0])
            lookahead_circle.set_radius(math.hypot(x_val, y_val))
    ...
    plot_image = plot_image.reshape(plot_shape) # 캔버스 크기는 고정이므로 프레임마다 get_width_height를 다시 호출하지 않음
